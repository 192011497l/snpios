    ...
    if ($check_stmt) {
        ...
        if ($check_stmt->num_rows > 0) {
            ...
            if ($password !== $confirm_password) {
                ...
                $insert_query = "INSERT INTO signup (username, email, mobile_no, shop_name, password) VALUES (?, ?, ?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_query);

                // Check if prepare() was successful
                if ($insert_stmt) {
                    $insert_stmt->bind_param("sssss", $username, $email, $mobile_no, $shop_name, $password);
